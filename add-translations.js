...
const langs = ['en', 'zh', 'de', 'ja', 'es'];

langs.forEach(lang => {
  // 1. Add geo_bold_1 after footer_social line
  const footerSocialMap = {
    en: 'YouTube & Social Media — Coming Soon',
    zh: 'YouTube 及自媒体 — 即将上线',
    de: 'YouTube & Social Media — Demnächst',
    ja: 'YouTube & ソーシャルメディア — 近日公開',
    es: 'YouTube y Redes Sociales — Próximamente'
  };
  const fsPattern = 'footer_social:"' + footerSocialMap[lang] + '",\n    },';
  const fsReplacement = 'footer_social:"' + footerSocialMap[lang] + '",\n      geo_bold_1:"' + geo[lang].replace(/"/g, '\\"') + '",\n    },';
  html = html.replace(fsPattern, fsReplacement);

  // 2. Add B2B FAQ before insights_cta_text
  const insightsCtaMap = {
    en: 'More industry analysis and product guides coming soon.',
    zh: '更多行业分析和产品指南即将发布。',
    de: 'Weitere Branchenanalysen und Produktführer kommen bald.',
    ja: 'より多くの業界分析と製品ガイドが近日公開。',
    es: 'Más análisis de la industria y guías de productos próximamente.'
  };
  const icPattern = 'insights_cta_text:"' + insightsCtaMap[lang] + '",)';
  const icReplacement = 'faq_b2b_1_q:"' + faq[lang].q1.replace(/"/g, '\\"') + '", faq_b2b_1_a:"' + faq[lang].a1.replace(/"/g, '\\"') + '", faq_b2b_2_q:"' + faq[lang].q2.replace(/"/g, '\\"') + '", faq_b2b_2_a:"' + faq[lang].a2.replace(/"/g, '\\"') + '", faq_b2b_3_q:"' + faq[lang].q3.replace(/"/g, '\\"') + '", faq_b2b_3_a:"' + faq[lang].a3.replace(/"/g, '\\"') + '", insights_cta_text:"' + insightsCtaMap[lang] + '",)';
  html = html.replace(icPattern, icReplacement);
});

fs.writeFileSync('index.html', html);
console.log('Done! Added translations to all 5 language blocks.');
